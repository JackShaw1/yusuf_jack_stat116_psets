...
__Since the numerator is the number of people that have not been diseased from the initial population, this simplifies to...__
...
This simplified variance is based on a Binomial distribution (look at the Variances). This makes sense because if there is no censoring, then we are essentially looking at the survival time of every different person at any given time as a Bernouli (any individual either has had the event or has not had the event at time) and therefore the collective survival curve is like a Binomial.
...
For race, we can see that the Dash diet group has a larger proportion of Hispanic,White, and Multiracial people and a much lower proportion of Black people than the non-Dash diet group. For age, we can see that the two groups are roughly similar, although the dash diet group is a little bit older in general. For BMI, we can see that the Dash diet seems to have slightly lower mean amd median BMI than the non-DASH diet group and also a much lower Max. For MI, we can see that the two groups are roughly the same (the Dash diet has a slightly higher proportion of people who have had Myocardial Infarction).  
...
```
As we can see, as time progresses the Dash diet group curve is consistently above the other curve, suggesting they do have a higher probability of survival. This difference is more pronounced towards the end. 
...
summary(cox1)
```
As we can see in the output, our p-value is now low and  we can reject the null. This tells us that after controlling for mi, chf, and bp, the dash diet is statistically significantly positively associated with survival. This is confirmed by our confidence interval, which now no lonnger contains 1, and our p-values. We can also see that all three of our covariates are statistically significant when controlling for the other covariates and diet.  
...
As we can see, our p-value for the diet variable is above 0.05 and therefore we fail to reject the null. We can not conclude that there is a statistically significant association between the Dash diet and survival when controlling for all 8 of these covariates of interest. However, a lot of these covariates are statistically significant when controlling for the others (all except bp). 
...
Firstly, we can see that from the p-values alone, there are potential violations for the covariates given that MI, age, sex, and bmi all have p-values below 0.05. However, actually looking at the plots, there don't seem to be major concerns that would lead us to think that our covariates have violations.
